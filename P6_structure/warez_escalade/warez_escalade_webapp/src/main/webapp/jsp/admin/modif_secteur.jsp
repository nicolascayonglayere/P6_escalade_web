...
<html>
	...
	<body>
		...
		<s:actionmessage/>
		<div class="container">
			<h3><s:text name="modifierTopo.topo"/> <s:property value="%{request['nomTopo']}"/></h3>
			<h4><s:text name="modifierTopo.site"/> <s:property value="%{param.nomSite}"/></h4>
			<h4><s:text name="modifierTopo.secteur"/> <s:property value="%{param['nomSecteur']}"/></h4>
				<s:form action="modifier_secteur" cssClass="form-horizontal" namespace="/jsp/utilisateur">
					...
					<s:textfield name="nomTopo" placeholder="${requestScope.nomTopo}" label="%{getText('form.nomTopo')}" requiredLabel="true"/>
					<s:submit class="btn btn-default" value="%{getText('bouton.valider')}">
	      				...
	      				<s:param name="nomTopo">${nomTopo}</s:param>
					</s:submit>
	     			<s:token/>
				</s:form>		
		</div>	
